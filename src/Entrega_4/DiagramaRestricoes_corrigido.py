
import numpy as np
from matplotlib import pyplot as plt

class AircraftEnvelope():
    
    def __init__(self):
        self.aircraft = {'W/S': [],
                         'P/W': []}
        

        # --- Performance
        self.W0_lb = 1172.2

        lb_to_kg = 0.453592

        self.W0_kg = self.W0_lb*lb_to_kg

        self.eta_prop = 0.8 # propeller efficiency

        # --- Statistical data
        self.P_W_cruise = 67.5 # [W/kg]
        self.P_W_climb = 66.6  # [W/kg]

        # --- Missions
        kmph_to_mps = 1/3.6

        self.rho_kgpm3_SL = 1.225 # at Sea Level
        self.rho_kgpm3_1000m = 1.1117 # at 1000m altitude

        self.V_cruise_mps = 250*kmph_to_mps
        self.Vmax_mps = 300*kmph_to_mps

        self.RoC_mps = 17 # missão 1, reconhecimento
        

        # --- Geometry
        self.AR = 14

        # --- Aerodynamics
        self.CLmax = 1.63 # Clark-Y

        self.e = 0.9 # wing efficiency
        
        self.CD0 = 0.03
        self.k = 1/(np.pi*self.AR*self.e)
        self.CD = self.CD0 + self.k*(self.CLmax**2)

        self.LDmax = 18
        self.LDmax_climb = self.LDmax # poderia assumir um valor de proporcionalidade
        
        k_stall = 3.5 # adjust stall curve to fit range of known aircraft
        self.V_stall_mps = np.sqrt( (2*self.W0_kg)/(self.rho_kgpm3_1000m*self.CLmax*k_stall))
        print(f'V_stall_mps = {self.V_stall_mps:.2f}')

class Sadraey_Methods(AircraftEnvelope):

    def __init__(self):
        super().__init__()

    def stall_restriction(self, rho_kgpm3):
        """
        Maximum W/S restriction based on Sadraey's equation 4.31

        --- --- ---
        INPUTS

        rho [kg/m3]

        V_stall_mps

        CLmax
        --- --- ---
        return W/S

        """
        W_S = 0.5*rho_kgpm3*(self.V_stall_mps**2)*self.CLmax

        return W_S
    
    
    def takeoff_restriction(self, W_S:np.array, S_TO, rho_kgpm3, mu, pitch_ratio = 0.6):
        """
        Take-off restriction equation based on Sadraey equation 4.75 
        
        --- --- ---
        INPUTS

        W_S = W/S array input [kg/m2]

        S_TO = take-off distance [m]

        mu = total rolling friction [-]

        pitch_ratio = 0.6 for variable pitch propeller or 0.5 for fixed pitch propeller

        """
        W_S_Npm2 = 9.81*W_S
        fixed_pitch_ratio = 0.5
        variable_pitch_ratio = 0.6

        V_TO_mps = 1.1 * self.V_stall_mps # FAR approximation
        CL_TO = 0.90 * self.CLmax
        CD_TO = self.CD0 + self.k*self.CLmax**2
        CD_G = CD_TO - mu*CL_TO

        K = pitch_ratio*rho_kgpm3*9.81*self.CD0*S_TO/W_S_Npm2
        
        num = mu - (mu + self.CD0/CL_TO)*( np.exp(K) )
        den = 1 - np.exp(K)

        P_W = (V_TO_mps/self.eta_prop)*(num/den) 

        return P_W * 9.81 # [W/kg]


    def maximumSpeed_restriction(self, W_S:np.array, rho_kgpm3):
        """
        Maximum speed restriction, Sadraey equation 4.56

        --- --- ---
        Usually, the maximum speed is assumed between 20% or 30% higher than the cruise speed.

        --- --- ---
        INPUTS

        W_S = W/S [kg/m^2]

        rho = local air density

        """
        W_S_Npm2 = 9.81*W_S

        Vmax_mps = 1.2*self.V_cruise_mps # used cruise speed as airspeed requirement for constant speed at specified altitude

        sigma = rho_kgpm3/self.rho_kgpm3_SL

        den = 0.5*self.rho_kgpm3_SL*(Vmax_mps**3)*self.CD0/W_S_Npm2 + (2*self.k)/(rho_kgpm3*sigma*Vmax_mps)*W_S_Npm2

        W_P = self.eta_prop/den

        return W_P / 9.81 # [kg/W]
    
    def RoC_restriction(self, W_S:np.array, RoC_mps, rho_kgpm3, eta_prop_climb = 0.7):
        """
        Rate of climb restriction, based on Sadraey 4.89

        --- --- ---
        INPUTS

        RoC_mps = rate of climb of the specified mission

        eta_prop_climb = propeller efficiency in climb, usually between 0.5 or 0.7 for a cruise-optimized propeller
        
        """
        W_S_Npm2 = 9.81*W_S

        P_W = (RoC_mps/eta_prop_climb) + np.sqrt( 2*W_S_Npm2/( rho_kgpm3 * np.sqrt(3*self.CD0/self.k )  ) ) * (1.155/(self.LDmax_climb*eta_prop_climb))

        return 1/P_W / 9.81
    
    def Ceiling_restriction(self, W_S:np.array, RoC_ceiling, rho_kgpm3_ceiling, eta_prop_climb = 0.7):
        """
        Absolute or service ceiling restriction, based on Sadraey equation 4.100

        Similar to the RoC restriction, however RoC depends on the type of ceiling defined.

        --- --- ---
        
        """
        W_S_Npm2 = 9.81*W_S

        sigma = rho_kgpm3_ceiling/self.rho_kgpm3_SL

        num = (RoC_ceiling/self.eta_prop) + np.sqrt( 2*W_S_Npm2/(rho_kgpm3_ceiling*np.sqrt(3*self.CD0/self.k) ) )*(1.155/(self.LDmax*self.eta_prop))

        P_W = num/sigma

        return P_W * 9.81
    
    def Range_restriction(self, W_S, rho_kgpm3, R_m = 500*10**3):
        """
        Equação deduzida
        
        """
        W_S_Npm2 = 9.81*W_S

        c_bhp = 0.068*10**(-6) # kg/Ws

        C = c_bhp*self.V_cruise_mps

        P_W = (self.eta_prop/(R_m*C))*np.log(1/0.9286)*np.sqrt(9.81*W_S_Npm2*2/(rho_kgpm3*self.CLmax))

        return P_W * 9.81
    
    def cruise_restriction(self, W_S, rho_kgpm3):
        """
        
        """
        self.V_cruise_mps = 300/3.6
        q = 0.5*rho_kgpm3*(self.V_cruise_mps**2)*20

        T_W = (q*self.CD0)/(W_S) + (self.k/q)*(W_S)

        return 1/(T_W*self.V_cruise_mps/(self.eta_prop))
    
    def show_RestrictionDiagram(self):
        plt.rcParams.update({
            'font.family': 'Arial',
            'font.size': 10,
            'axes.linewidth': 0.8,
            'axes.grid': True,
            'grid.linestyle': '--',
            'grid.linewidth': 0.4,
            'grid.alpha': 0.5,
            'legend.framealpha': 0.95,
            'legend.edgecolor': '0.6',
            'legend.fontsize': 9,
        })

        fig, ax = plt.subplots(figsize=(8, 6), dpi=150)

        W_S_stall = self.stall_restriction(rho_kgpm3=1.225)
        W_S_array = np.linspace(0.5, W_S_stall, 1000)

        # --- Paleta monocromática
        cores = {
            'estol':     ('#000000', ':',          1.5),
            'decolagem': ('#000000', '-',          2.0),
            'cruzeiro':  ('#444444', '-',         2.0),
            'subida':    ('#222222', (0, (5, 2)),  2.0),
            'teto':      ('#777777', '-.',         1.8),
        }

        # --- 1) Estol
        ax.axvline(
            x=W_S_stall,
            linestyle=cores['estol'][1],
            linewidth=cores['estol'][2],
            color=cores['estol'][0],
            label='Limite de estol',
            zorder=5
        )

        # --- 2) Decolagem
        P_W_takeoff = self.takeoff_restriction(
            W_S_array, S_TO=150, rho_kgpm3=1.1117, mu=0.03)
        ax.plot(
            W_S_array, P_W_takeoff,
            linestyle=cores['decolagem'][1],
            linewidth=cores['decolagem'][2],
            color=cores['decolagem'][0],
            label='Decolagem',
            zorder=4
        )

        # --- 3) Cruzeiro
        PWcruise_to_PWtakeoff = self.V_cruise_mps/(745.7*self.eta_prop*self.LDmax) * self.P_W_cruise
        W_P_maxspeed = self.maximumSpeed_restriction(W_S_array, rho_kgpm3=1.1117)
        ax.plot(
            W_S_array, 1/W_P_maxspeed*PWcruise_to_PWtakeoff,
            linestyle=cores['cruzeiro'][1],
            linewidth=cores['cruzeiro'][2],
            color=cores['cruzeiro'][0],
            label='Cruzeiro',
            zorder=4,
            marker='x', markevery=50, markersize=5,
        )

        # --- 4) Taxa de subida
        PWclimb_to_PWtakeoff = 1.1*self.V_stall_mps/(745.7*self.eta_prop*self.LDmax) * self.P_W_climb
        W_P_roc = self.RoC_restriction(W_S_array, RoC_mps=17, rho_kgpm3=1.1117)
        ax.plot(
            W_S_array, 1/W_P_roc*PWclimb_to_PWtakeoff,
            linestyle=cores['subida'][1],
            linewidth=cores['subida'][2],
            color=cores['subida'][0],
            label='Taxa de subida',
            zorder=4
        )

        # --- 5) Teto absoluto
        PWceiling_to_PWtakeoff = self.V_cruise_mps/(745.7*self.eta_prop*self.LDmax) * self.P_W_climb
        P_W_ceiling = self.Ceiling_restriction(
            W_S_array, RoC_ceiling=0.508, rho_kgpm3_ceiling=1.0066)
        ax.plot(
            W_S_array, P_W_ceiling*PWceiling_to_PWtakeoff,
            linestyle=cores['teto'][1],
            linewidth=cores['teto'][2],
            color=cores['teto'][0],
            label='Teto de serviço',
            zorder=4
        )


        # --- Sombreamento monocromático
        P_W_lower_bound = np.maximum.reduce([
            P_W_takeoff,        # já calculado antes do plot de cruzeiro
            1/W_P_maxspeed*PWcruise_to_PWtakeoff,        # já calculado antes do plot de curva
        ])

        ax.fill_between(
            W_S_array,
            P_W_lower_bound,
            200,
            alpha=0.2,
            color='#888888',
            label='Região viável'
        )
        # --- Formatação dos eixos
        ax.set_xlabel(r'$W/S$  [kg/m²]', fontsize=11)
        ax.set_ylabel(r'$P/W$  [W/kg]',  fontsize=11)
        ax.set_ylim(0, 150)
        ax.tick_params(direction='in', length=4, width=0.6, top=True, right=True)

        # --- Eixos secundários (unidades imperiais)
        # Fatores: 1hp = 745.7W, 1kg = 2.2046lb, 1m = 3.28084ft
        secax_x = ax.secondary_xaxis(
            'top',
            functions=(
                lambda x: x * 2.2046 / 3.28084**2,
                lambda x: x / 2.2046 * 3.28084**2
            )
        )
        secax_x.set_xlabel(r'$W/S$  [lb/ft²]', fontsize=11)
        secax_x.tick_params(direction='in', length=4, width=0.6)

        secax_y = ax.secondary_yaxis(
            'right',
            functions=(
                lambda y: y / 745.7 / 2.2046,
                lambda y: y * 745.7 * 2.2046
            )
        )
        secax_y.set_ylabel(r'$P/W$  [hp/lb]', fontsize=11)
        secax_y.tick_params(direction='in', length=4, width=0.6)

        #ax.scatter(70, 120.6, marker='*', s=80, color='blue', label='Ponto de Projeto')
        #ax.plot([0, W_S_stall], [120.6, 120.6], label='Motor T422', linestyle=':', linewidth=1.5, color='blue')
        #ax.plot([70, 70], [0, 120.6], label='Carga-alar de projeto', linestyle='-.', linewidth=1.5, color='blue')

        # --- Legenda
        ax.legend(
            loc='upper left',
            frameon=True,
            ncol=1,
            title='Restrições de projeto',
            title_fontsize=9,
            handlelength=2.5
        )

        # --- Título
        ax.set_title(
            'Diagrama de Restrições — Metodologia Sadraey',
            fontsize=11, fontweight='bold', loc='center', pad=10
        )

        # --- Grade secundária
        ax.minorticks_on()
        ax.grid(which='minor', linestyle=':', linewidth=0.2, alpha=0.4)

        fig.tight_layout()
        fig.savefig('RestrictionsDiagram_Sadraey.pdf', dpi=300, bbox_inches='tight')        #plt.show()


class Gudmundsson_Methods(AircraftEnvelope):

    def __init__(self):
        super().__init__()

    def stall_restriction(self, rho_kgpm3):
        """
        Maximum W/S restriction based on Raymer's equation 5.6

        --- --- ---
        INPUTS

        rho [kg/m3]

        V_stall_mps

        CLmax
        --- --- ---
        return W/S

        """
        W_S = 0.5*rho_kgpm3*(self.V_stall_mps**2)*self.CLmax

        return W_S
    
    def takeoff_restriction(self, W_S:np.array, S_G_m, rho_kgpm3, mu) -> np.array:
        """
        Take-off curve restriction, based on Gudmundsson's equation 3-4
        

        --- --- ---
        S_G_m = total take-off distance [m]

        mu = total ground roll friction [-]

        rho_kgpm3 = local air density
        """
        W_S_Npm2 = 9.81*W_S
        V_LOF_mps = 1.1 * self.V_stall_mps # FAR 23 approximation
        CL_TO = 0.9 * self.CLmax           # FAR 23 approximation

        q_Pa = 0.5*rho_kgpm3*V_LOF_mps**2

        T_W = (V_LOF_mps**2)/(2*9.81*S_G_m) + (q_Pa*self.CD/W_S_Npm2) + mu*(1 - (q_Pa*CL_TO)/W_S_Npm2)

        # --- converting to power
        P_W = T_W * 9.81 * V_LOF_mps/self.eta_prop # [W/kg]

        return P_W
    
    def takeoff_parameter_restriction(self, W_S:np.array, rho_kgpm3, TOP = 200):
        '''
        Restriction based on TOP obtained by Raymer's figure 5.4. Equation 5.8
        '''
        W_S_Npm2 = 9.81*W_S

        CL_TO = 0.9 * self.CLmax           # FAR 23 approximation

        sigma = rho_kgpm3/1.225

        P_W = W_S_Npm2/(TOP*sigma*CL_TO) * 9.81

        return P_W
    
    def cruise_restriction(self, W_S:np.array, rho_kgpm3):
        """
        Cruise airspeed restriction, based on Gudmundsson equation 3-5
        
        --- --- ---
        rho_kgpm3 = local air density
        """
        W_S_Npm2 = 9.81*W_S

        q_Pa = 0.5 * rho_kgpm3 * self.V_cruise_mps**2

        T_W = q_Pa*self.CD0/W_S_Npm2 + self.k/q_Pa*W_S_Npm2

        # --- converting to power
        P_W = T_W * 9.81 * self.V_cruise_mps/self.eta_prop

        return P_W 
    
    def service_ceiling_restriction(self, W_S:np.array, rho_kgpm3, RoC_mps = 0.508):
        """
        Service ceiling restriction, based on Gudmundsson equation 3-6

        --- --- ---
        rho_kgpm3 = local air density
        """
        W_S_Npm2 = 9.81*W_S

        T_W = RoC_mps/(np.sqrt( (2/rho_kgpm3)*W_S_Npm2*np.sqrt( self.k/(3*self.CD))    )) + 4*np.sqrt(self.k*self.CD/3)

        # --- converting to power
        P_W = T_W * 9.81 * self.V_cruise_mps/self.eta_prop

        return P_W
    
    def climb_restriction(self, W_S:np.array, rho_kgpm3, RoC_mps):
        """
        Rate of climb restriction, based on Gudmundsson equation 3-6

        --- --- ---
        rho_kgpm3 = local air density
        """
        W_S_Npm2 = 9.81*W_S

        q_Pa = 0.5*rho_kgpm3*(1.1*self.V_stall_mps)**2 # during V_LOF

        V_climb_mps = self.V_cruise_mps

        T_W = RoC_mps/V_climb_mps + q_Pa/W_S_Npm2*self.CD + self.k/q_Pa*W_S_Npm2

        # --- converting to power
        P_W = T_W * 9.81 *self.V_cruise_mps/self.eta_prop

        return P_W
    
    def sustained_turn_restriction(self, W_S:np.array, rho_kgpm3, bank_angle_deg):
        """
        Level-constant velocity turn restriction, based on Gudmundsson equation 3-6

        --- --- ---
        rho_kgpm3 = local air density
        """
        W_S_Npm2 = 9.81*W_S

        q_Pa = 0.5*rho_kgpm3*(1.1*self.V_stall_mps)**2

        n = 1/np.cos( np.deg2rad(bank_angle_deg) )

        T_W = q_Pa*(self.CD/W_S_Npm2 + self.k*W_S_Npm2*(n/q_Pa)**2)

        # --- converting to power
        P_W = T_W * 9.81 * self.V_cruise_mps/self.eta_prop

        return P_W

    def show_RestrictionDiagram(self):
        plt.rcParams.update({
            'font.family': 'Arial',
            'font.size': 10,
            'axes.linewidth': 0.8,
            'axes.grid': True,
            'grid.linestyle': '--',
            'grid.linewidth': 0.4,
            'grid.alpha': 0.5,
            'legend.framealpha': 0.95,
            'legend.edgecolor': '0.6',
            'legend.fontsize': 9,
        })

        fig, ax = plt.subplots(figsize=(8, 6), dpi=150)

        W_S_stall = self.stall_restriction(rho_kgpm3=1.1117)
        W_S_array = np.linspace(0.5, W_S_stall, 1000)

        # --- Paleta monocromática 
        cores = {
            'estol':      ('#000000', ':',   1.5),
            'decolagem':  ('#000000', '-',   2.0),
            'cruzeiro':   ('#444444', '-',  2.0),
            'teto':       ('#777777', '-.',  1.8),
            'subida':     ('#222222', (0,(5,2)), 2.0),
            'curva':      ('#555555', '-', 2.0),
        }

        # --- 1) Estol
        ax.axvline(
            x=W_S_stall,
            linestyle=cores['estol'][1],
            linewidth=cores['estol'][2],
            color=cores['estol'][0],
            label='Limite de estol',
            zorder=5
        )

        # --- 2) Decolagem
        P_W_takeoff = self.takeoff_restriction(W_S_array, S_G_m=150, rho_kgpm3=1.1117, mu=0.03)
        ax.plot(
            W_S_array, P_W_takeoff,
            linestyle=cores['decolagem'][1],
            linewidth=cores['decolagem'][2],
            color=cores['decolagem'][0],
            label='Decolagem (Distância máxima)',
            zorder=4
        )

        P_W_TOP = self.takeoff_parameter_restriction(W_S_array, rho_kgpm3=1.1117, TOP = 120)
        ax.plot(
            W_S_array, P_W_TOP,
            linestyle=cores['decolagem'][1],
            linewidth=cores['decolagem'][2],
            color=cores['decolagem'][0],
            label='Decolagem (Rolagem)',
            zorder=4,
            marker='^', markevery=50, markersize=5
        )

        # --- 3) Cruzeiro

        PWcruise_to_PWtakeoff = self.V_cruise_mps/(745.7*self.eta_prop*self.LDmax) * self.P_W_cruise
        P_W_cruise = self.cruise_restriction(W_S_array, rho_kgpm3=1.1117)
        ax.plot(
            W_S_array, P_W_cruise*PWcruise_to_PWtakeoff,
            linestyle=cores['cruzeiro'][1],
            linewidth=cores['cruzeiro'][2],
            color=cores['cruzeiro'][0],
            label='Cruzeiro',
            zorder=4,
            marker='x', markevery=50, markersize=5
        )

        # --- 4) Teto de serviço
        PWceiling_to_PWtakeoff = self.V_cruise_mps/(745.7*self.eta_prop*self.LDmax) * self.P_W_climb
        P_W_ceiling = self.service_ceiling_restriction(W_S_array, rho_kgpm3=1.0066)
        ax.plot(
            W_S_array, P_W_ceiling*PWceiling_to_PWtakeoff,
            linestyle=cores['teto'][1],
            linewidth=cores['teto'][2],
            color=cores['teto'][0],
            #marker='x', markevery=50, markersize=5,
            label='Teto de serviço',
            zorder=4
        )

        # --- 5) Taxa de subida
        PWclimb_to_PWtakeoff = 1.1*self.V_stall_mps/(745.7*self.eta_prop*self.LDmax) * self.P_W_climb
        P_W_roc = self.climb_restriction(W_S_array, rho_kgpm3=1.1117, RoC_mps=17)
        ax.plot(
            W_S_array, P_W_roc*PWclimb_to_PWtakeoff,
            linestyle=cores['subida'][1],
            linewidth=cores['subida'][2],
            color=cores['subida'][0],
            label='Taxa de subida',
            zorder=4
        )

        # --- 6) Curva sustentada
        PWcurve_to_PWtakeoff = 1.2*self.V_stall_mps/(745.7*self.eta_prop*self.LDmax) * self.P_W_cruise
        P_W_curve = self.sustained_turn_restriction(W_S_array, rho_kgpm3=1.1117, bank_angle_deg=45)
        ax.plot(
            W_S_array, P_W_curve*PWcurve_to_PWtakeoff,
            linestyle=cores['curva'][1],
            linewidth=cores['curva'][2],
            color=cores['curva'][0],
            label='Curva sustentada',
            zorder=4,
            marker='s', markevery=50, markersize=5
        )

        #ax.scatter(70, 120.6, marker='*', s=80, color='blue', label='Ponto de Projeto')
        #ax.plot([0, W_S_stall], [120.6, 120.6], label='Motor T422', linestyle=':', linewidth=1.5, color='blue')
        #ax.plot([70, 70], [0, 120.6], label='Carga-alar de projeto', linestyle='-.', linewidth=1.5, color='blue')

        # --- Formatação dos eixos
        ax.set_xlabel(r'$W/S$  [kg/m²]', fontsize=11)
        ax.set_ylabel(r'$P/W$  [W/kg]',  fontsize=11)
        ax.set_ylim(0, 200)
        ax.tick_params(direction='in', length=4, width=0.6, top=True, right=True)

        # --- Eixos secundários 
        # 1hp = 745.7W, 1kg = 2.2046lb, 1m = 3.28084ft

        secax_x = ax.secondary_xaxis(
            'top',
            functions=(
                lambda x: x * 2.2046 / 3.28084**2,   # kg/m² → lb/ft²
                lambda x: x / 2.2046 * 3.28084**2    # lb/ft² → kg/m²
            )
        )
        secax_x.set_xlabel(r'$W/S$  [lb/ft²]', fontsize=11)
        secax_x.tick_params(direction='in', length=4, width=0.6)

        secax_y = ax.secondary_yaxis(
            'right',
            functions=(
                lambda y: y / 745.7 / 2.2046,   # W/kg → hp/lb
                lambda y: y * 745.7 * 2.2046    # hp/lb → W/kg
            )
        )
        secax_y.set_ylabel(r'$P/W$  [hp/lb]', fontsize=11)
        secax_y.tick_params(direction='in', length=4, width=0.6)

        # --- Região viável (acima de cruzeiro, curva sustentada e teto de serviço)
        P_W_lower_bound = np.maximum.reduce([
            P_W_cruise * PWcruise_to_PWtakeoff,        # já calculado antes do plot de cruzeiro
            P_W_curve*PWcurve_to_PWtakeoff,        # já calculado antes do plot de curva
            P_W_ceiling * PWceiling_to_PWtakeoff,        # já calculado antes do plot de teto
            P_W_TOP
        ])

        ax.fill_between(
            W_S_array,
            P_W_lower_bound,
            200,
            alpha=0.2,
            color='#888888',
            label='Região viável'
        )
        # --- Legenda
        ax.legend(
            loc='upper left',
            frameon=True,
            ncol=1,
            title='Restrições de projeto',
            title_fontsize=9,
            handlelength=2.5
        )

        # --- Título
        ax.set_title(
            'Diagrama de Restrições — Metodologia Gudmundsson',
            fontsize=11, fontweight='bold', loc='center', pad=10
        )

        fig.tight_layout()
        fig.savefig('RestrictionsDiagram_Gudmundsson.pdf', dpi=300, bbox_inches='tight')
        plt.show()

def compare_diagrams():
        gudmundsson = Gudmundsson_Methods()
        sadraey = Sadraey_Methods()

        plt.rcParams.update({
                'font.family': 'Arial',
                'font.size': 10,
                'axes.linewidth': 0.8,
                'axes.grid': True,
                'grid.linestyle': '--',
                'grid.linewidth': 0.4,
                'grid.alpha': 0.5,
                'legend.framealpha': 0.95,
                'legend.edgecolor': '0.6',
                'legend.fontsize': 9,
            })

        fig, ax = plt.subplots(figsize=(10, 6), dpi=150)

        W_S_stall = gudmundsson.stall_restriction(rho_kgpm3=1.1117)
        W_S_array = np.linspace(0.5, W_S_stall, 1000)

        # --- Paleta monocromática 
        cores = {
            'estol':      ('#000000', ':',   1.5),
            'decolagem':  ('#000000', '-',   2.0),
            'cruzeiro':   ('#444444', '-',  2.0),
            'teto':       ('#777777', '-.',  1.8),
            'subida':     ('#222222', (0,(5,2)), 2.0),
            'curva':      ('#555555', '-', 2.0),
        }

        # --- 1) Estol
        ax.axvline(
            x=W_S_stall,
            linestyle=cores['estol'][1],
            linewidth=cores['estol'][2],
            color=cores['estol'][0],
            label='Limite de estol',
            zorder=5
        )

        # --- 2) Decolagem
        P_W_takeoff = gudmundsson.takeoff_restriction(W_S_array, S_G_m=150, rho_kgpm3=1.1117, mu=0.03)
        ax.plot(
            W_S_array, P_W_takeoff,
            linestyle=cores['decolagem'][1],
            linewidth=cores['decolagem'][2],
            color=cores['decolagem'][0],
            label='Decolagem (Distância máxima)',
            zorder=4
        )

        P_W_TOP = gudmundsson.takeoff_parameter_restriction(W_S_array, rho_kgpm3=1.1117, TOP = 120)
        ax.plot(
            W_S_array, P_W_TOP,
            linestyle=cores['decolagem'][1],
            linewidth=cores['decolagem'][2],
            color=cores['decolagem'][0],
            label='Decolagem (Rolagem)',
            zorder=4,
            marker='^', markevery=50, markersize=5
        )

        # --- 3) Cruzeiro

        PWcruise_to_PWtakeoff = gudmundsson.V_cruise_mps/(745.7*gudmundsson.eta_prop*gudmundsson.LDmax) * gudmundsson.P_W_cruise
        P_W_cruise = gudmundsson.cruise_restriction(W_S_array, rho_kgpm3=1.1117)
        ax.plot(
            W_S_array, P_W_cruise*PWcruise_to_PWtakeoff,
            linestyle=cores['cruzeiro'][1],
            linewidth=cores['cruzeiro'][2],
            color=cores['cruzeiro'][0],
            label='Cruzeiro',
            zorder=4,
            marker='x', markevery=50, markersize=5
        )

        # --- 4) Teto de serviço
        PWceiling_to_PWtakeoff_G = gudmundsson.V_cruise_mps/(745.7*gudmundsson.eta_prop*gudmundsson.LDmax) * gudmundsson.P_W_climb
        P_W_ceiling_G = gudmundsson.service_ceiling_restriction(W_S_array, rho_kgpm3=1.0066)
        ax.plot(
            W_S_array, P_W_ceiling_G*PWceiling_to_PWtakeoff_G,
            linestyle=cores['teto'][1],
            linewidth=cores['teto'][2],
            color=cores['teto'][0],
            #marker='x', markevery=50, markersize=5,
            label='Teto de serviço',
            zorder=4
        )

        # --- 5) Taxa de subida
        PWclimb_to_PWtakeoff = 1.1*gudmundsson.V_stall_mps/(745.7*gudmundsson.eta_prop*gudmundsson.LDmax) * gudmundsson.P_W_climb
        P_W_roc = gudmundsson.climb_restriction(W_S_array, rho_kgpm3=1.1117, RoC_mps=17)
        ax.plot(
            W_S_array, P_W_roc*PWclimb_to_PWtakeoff,
            linestyle=cores['subida'][1],
            linewidth=cores['subida'][2],
            color=cores['subida'][0],
            label='Taxa de subida',
            zorder=4
        )
    

        # --- 6) Curva sustentada
        PWcurve_to_PWtakeoff = 1.2*gudmundsson.V_stall_mps/(745.7*gudmundsson.eta_prop*gudmundsson.LDmax) * gudmundsson.P_W_cruise
        P_W_curve = gudmundsson.sustained_turn_restriction(W_S_array, rho_kgpm3=1.1117, bank_angle_deg=45)
        ax.plot(
            W_S_array, P_W_curve*PWcurve_to_PWtakeoff,
            linestyle=cores['curva'][1],
            linewidth=cores['curva'][2],
            color=cores['curva'][0],
            label='Curva sustentada',
            zorder=4,
            marker='s', markevery=50, markersize=5
        )
    
        # ----- Sadraey
        W_S_array = np.linspace(0.5, W_S_stall, 1000)

        # --- Paleta monocromática
        cores = {
            'estol':     ('#5DADE2', ':',          1.5),   # azul claro
            'decolagem': ('#3498DB', '-',          2.2),   # azul médio
            'cruzeiro':  ('#85C1E9', '--',         2.0),   # azul pastel
            'subida':    ('#2E86C1', (0, (5, 2)), 2.0),   # azul mais intenso
            'teto':      ('#AED6F1', '-.',         1.8),   # azul muito claro
        }

        # --- 2) Decolagem
        P_W_takeoff = sadraey.takeoff_restriction(
            W_S_array, S_TO=150, rho_kgpm3=1.1117, mu=0.03)
        ax.plot(
            W_S_array, P_W_takeoff,
            linestyle=cores['decolagem'][1],
            linewidth=cores['decolagem'][2],
            color=cores['decolagem'][0],
            label='Decolagem (FAR-23)',
            zorder=4
        )

        # --- 3) Cruzeiro
        PWcruise_to_PWtakeoff_S = sadraey.V_cruise_mps/(745.7*sadraey.eta_prop*sadraey.LDmax) * sadraey.P_W_cruise
        W_P_maxspeed = sadraey.maximumSpeed_restriction(W_S_array, rho_kgpm3=1.1117)
        ax.plot(
            W_S_array, 1/W_P_maxspeed*PWcruise_to_PWtakeoff_S,
            linestyle=cores['cruzeiro'][1],
            linewidth=cores['cruzeiro'][2],
            color=cores['cruzeiro'][0],
            label='Cruzeiro (FAR-23)',
            zorder=4,
            marker='x', markevery=50, markersize=5,
        )

        # --- 4) Taxa de subida
        PWclimb_to_PWtakeoff_S = 1.1*sadraey.V_stall_mps/(745.7*sadraey.eta_prop*sadraey.LDmax) * sadraey.P_W_climb
        W_P_roc = sadraey.RoC_restriction(W_S_array, RoC_mps=17, rho_kgpm3=1.1117)
        ax.plot(
            W_S_array, 1/W_P_roc*PWclimb_to_PWtakeoff_S,
            linestyle=cores['subida'][1],
            linewidth=cores['subida'][2],
            color=cores['subida'][0],
            label='Taxa de subida (FAR-23)',
            zorder=4
        )

        # --- 5) Teto absoluto
        PWceiling_to_PWtakeoff_S = sadraey.V_cruise_mps/(745.7*sadraey.eta_prop*sadraey.LDmax) * sadraey.P_W_climb
        P_W_ceiling = sadraey.Ceiling_restriction(
            W_S_array, RoC_ceiling=0.508, rho_kgpm3_ceiling=1.0066)
        ax.plot(
            W_S_array, P_W_ceiling*PWceiling_to_PWtakeoff_S,
            linestyle=cores['teto'][1],
            linewidth=cores['teto'][2],
            color=cores['teto'][0],
            label='Teto de serviço (FAR-23)',
            zorder=4
        )

        # --------

        #ax.scatter(70, 120.6, marker='*', s=80, color='blue', label='Ponto de Projeto')
        #ax.plot([0, W_S_stall], [120.6, 120.6], label='Motor T422', linestyle=':', linewidth=1.5, color='blue')
        #ax.plot([70, 70], [0, 120.6], label='Carga-alar de projeto', linestyle='-.', linewidth=1.5, color='blue')

        # --- Formatação dos eixos
        ax.set_xlabel(r'$W/S$  [kg/m²]', fontsize=11)
        ax.set_ylabel(r'$P/W$  [W/kg]',  fontsize=11)
        ax.set_ylim(0, 125)
        ax.tick_params(direction='in', length=4, width=0.6, top=True, right=True)

        # --- Eixos secundários 
        # 1hp = 745.7W, 1kg = 2.2046lb, 1m = 3.28084ft

        secax_x = ax.secondary_xaxis(
            'top',
            functions=(
                lambda x: x * 2.2046 / 3.28084**2,   # kg/m² → lb/ft²
                lambda x: x / 2.2046 * 3.28084**2    # lb/ft² → kg/m²
            )
        )
        secax_x.set_xlabel(r'$W/S$  [lb/ft²]', fontsize=11)
        secax_x.tick_params(direction='in', length=4, width=0.6)

        secax_y = ax.secondary_yaxis(
            'right',
            functions=(
                lambda y: y / 745.7 / 2.2046,   # W/kg → hp/lb
                lambda y: y * 745.7 * 2.2046    # hp/lb → W/kg
            )
        )
        secax_y.set_ylabel(r'$P/W$  [hp/lb]', fontsize=11)
        secax_y.tick_params(direction='in', length=4, width=0.6)

        # --- Região viável (acima de cruzeiro, curva sustentada e teto de serviço)
        P_W_lower_bound = np.maximum.reduce([
            P_W_cruise * PWcruise_to_PWtakeoff,
            P_W_curve * PWcurve_to_PWtakeoff,
            P_W_ceiling_G * PWceiling_to_PWtakeoff_G,
            P_W_roc * PWclimb_to_PWtakeoff,
            P_W_TOP
        ])

        ax.fill_between(
            W_S_array,
            P_W_lower_bound,
            200,
            alpha=0.2,
            color='#888888',
            label='Região viável (Metodologia Gudmundsson)'
        )
        
        # --- Sombreamento monocromático
        P_W_lower_bound = np.maximum.reduce([
            P_W_takeoff,        # já calculado antes do plot de cruzeiro
            1/W_P_maxspeed*PWcruise_to_PWtakeoff_S,        # já calculado antes do plot de curva
        ])

        ax.fill_between(
            W_S_array,
            P_W_lower_bound,
            200,
            alpha=0.2,
            color="#3330302F",
            label='Região viável (Metodologia Sadraey)'
        )

        # --- Legenda
        ax.legend(
            loc='upper left',
            frameon=True,
            ncol=1,
            title='Restrições de projeto',
            title_fontsize=9,
            handlelength=2.5
        )


        # --- Título
        ax.set_title(
            'Diagrama de Restrições — Metodologia Gudmundsson + Sadraey',
            fontsize=11, fontweight='bold', loc='center', pad=10
        )

        fig.tight_layout()
        fig.savefig('MergedRestrictionsDiagram.pdf', dpi=300, bbox_inches='tight')
        plt.show()



diagram_sadraey =  Sadraey_Methods()
diagram_sadraey.show_RestrictionDiagram()

diagram_Gudmundsson = Gudmundsson_Methods()
diagram_Gudmundsson.show_RestrictionDiagram()

compare_diagrams()


